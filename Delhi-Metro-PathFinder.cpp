...
// Structure to represent a metro connection between stations
struct Connection {
    string destination;  // Name of the connected station
    int time;           // Travel time in minutes
    string line;        // Metro line color/name
...
struct Node {
    int distance;       // Total distance/time from source
    string station;     // Current station name
    string currentLine; // Current metro line we're on
...
class MetroSystem {
private:
    // Graph: station name -> list of connections
    map<string, vector<Connection> > graph;
    
    // Which lines are available at each station (for interchange info)
    map<string, set<string> > stationLines;
    
    // Penalty in minutes for changing metro lines
...
public:
    ...
    // Set the penalty time for changing lines
    ...
    pair<int, vector<string> > findShortestPath(string start, string end) {
        ...
        // Track parent station and line used to reach each station
        map<string, pair<string, string> > parent;
        
        // Priority queue: processes stations in order of shortest distance
        priority_queue<Node, vector<Node>, greater<Node> > pq;
        
        // Initialize all distances to infinity
        for (map<string, vector<Connection> >::iterator it = graph.begin(); it != graph.end(); ++it) {
            dist[it->first] = INT_MAX;
        }
        
        // Starting station has distance 0
        dist[start] = 0;
        ...
        while (!pq.empty()) {
            ...
            if (currentDist > dist[currentStation]) {
                continue;
            }
            
            // Check all neighboring stations
            vector<Connection>& connections = graph[currentStation];
            for (size_t i = 0; i < connections.size(); i++) {
                Connection& conn = connections[i];
                int edgeWeight = conn.time;
                
                // Add penalty if changing lines (not at start)
                if (!currentLine.empty() && currentLine != conn.line) {
                ...
                if (newDist < dist[conn.destination]) {
                    ...
                    parent[conn.destination] = make_pair(currentStation, conn.line);
        ...
        vector<string> path;
        if (dist[end] == INT_MAX) {
            return make_pair(-1, path); // No path exists
        }
    ...
    void displayRoute(string start, string end) {
        ...
        // Reconstruct route with line information
        map<string, pair<string, string> > parent;
        map<string, int> dist;
        priority_queue<Node, vector<Node>, greater<Node> > pq;
            ...
            if (i == 0) {
                ...
                if (i == path.size() - 1) {
                    ...
                    if (stationLines[path[i]].size() > 1) {
                        cout << " [INTERCHANGE: ";
                        bool first = true;
                        for (set<string>::iterator it = stationLines[path[i]].begin(); 
                             it != stationLines[path[i]].end(); ++it) {
                            if (!first) cout << ", ";
                            cout << *it;
                            first = false;
                        }
    ...
    void listAllStations() {
        ...
        for (size_t i = 0; i < stations.size(); i++) {
            ...
            cout << " [";
            bool first = true;
            for (set<string>::iterator it = stationLines[stations[i]].begin(); 
                 it != stationLines[stations[i]].end(); ++it) {
                if (!first) cout << ", ";
                cout << *it;
                first = false;
            }
...
// Function to initialize complete Delhi Metro network
void initializeDelhiMetro(MetroSystem& metro) {
    ...
    // BLUE LINE (Line 3 & 4) - Dwarka to Noida/Vaishali
    ...
    // GREEN LINE (Line 5) - Kirti Nagar to Brigadier Hoshiar Singh
    ...
    // AQUA LINE (Noida-Greater Noida) - Noida Sector 51 to Depot Station
    ...
    // RAPID METRO (Gurgaon) - Sikanderpur to Cyber City
